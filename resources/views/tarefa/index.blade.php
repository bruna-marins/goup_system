...
<div class="container">
    ...
    <div class="d-flex justify-content-between mb-4">
        <a href="{{ route('tarefa.index', ['mes' => $mesAnterior->format('Y-m')]) }}" class="btn btn-secondary">
            Mês Anterior ({{ $mesAnterior->format('F') }})
        </a>
        <a href="{{ route('tarefa.index', ['mes' => $mesProximo->format('Y-m')]) }}" class="btn btn-secondary">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @for($i = 1; $i <= $diasNoMes; $i++)
                            ...
                            <td>
                                <a href="{{ route('tarefa.create', $dataAtual) }}" class="btn btn-light">
                                    {{ $i }} <!-- Exibe o número do dia -->
                                </a> <br>
                                <a href="{{ route('tarefa.show', $dataAtual) }}">
                                    @if($quantidadeTarefas > 0)
                                        <!-- Exibe a quantidade de tarefas para o dia -->
                            ...
                            @if(($i + $diaDaSemanaPrimeiroDia) % 7 == 0)
                                </tr><tr> <!-- Fecha e abre uma nova linha -->
                            @endif
                        @endfor
